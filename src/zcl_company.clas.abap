class zcl_company definition
  ...
  private section.
    ...
    data:
      mo_company_master_data   type ref to zif_bp_master_data,
      mo_company_contacts_book type ref to zif_contacts_book.
...
class zcl_company implementation.
  ...
  method constructor.
    ...
    mo_company_master_data = new zcl_bp_master_data( ip_bp_num ).
    mo_company_contacts_book ?= mo_company_master_data.

  endmethod.

  method zif_company~get_company_bp_number.

    rp_bp_num = me->mo_company_master_data->get_bp_number(  ).

  endmethod.

  method zif_company~get_company_bp_address_string.
  ...
  method zif_company~get_company_email_address.

    rp_bp_email_address = me->mo_company_contacts_book->get_email_address(  ).

  endmethod.

  method zif_company~get_company_name.

    rp_bp_company_name = me->mo_company_master_data->get_but000_record(  )-mc_name1.

  endmethod.

  method zif_company~get_contact_persons.
