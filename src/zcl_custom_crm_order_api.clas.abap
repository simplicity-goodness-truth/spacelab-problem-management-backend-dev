class zcl_custom_crm_order_api definition
  public
  create public .
...
class zcl_custom_crm_order_api implementation.
  ...
  method zif_custom_crm_order_read~get_all_priorities_list.
    ...
    data: ls_priority     type zcrm_order_ts_priority,
  ...
  method zif_custom_crm_order_read~get_standard_fields_by_guid.
    ...
    data:
      ...
      lo_categorization_schema type ref to zif_crm_categorization_schema,
    ...
    if sy-subrc <> 0.
      ...
      raise exception type zcx_crm_order_api_exc
        exporting
          textid  = zcx_crm_order_api_exc=>cant_get_document_by_guid
          ip_guid = ip_guid.
...
*    endif.
  ...
  method zif_custom_crm_order_read~get_custom_fields_by_guid.
    ...
    field-symbols: <ls_db_struct_fields_map> like line of mt_db_struct_fields_map,
                   <lv_value>                type any,
    ...
    loop at mt_db_struct_fields_map assigning <ls_db_struct_fields_map>.
      ...
      if lv_db_fields_list is initial.
        ...
        concatenate lv_db_fields_list <ls_db_struct_fields_map>-db_field into lv_db_fields_list separated by space.
  ...
  method zif_custom_crm_order_create~create_with_std_and_cust_flds.
    ...
    data:
      ...
      lv_rc                   type sy-subrc,
      lt_custom_fields_map    type zcrm_order_tt_cust_fields_map,
    ...
    if mv_process_type is initial.
      raise exception type zcx_crm_order_api_exc
        exporting
          textid = zcx_crm_order_api_exc=>process_type_not_set.
    ...
    if lo_cl_ags_crm_1o_api is not bound.
      raise exception type zcx_crm_order_api_exc
        exporting
          textid = zcx_crm_order_api_exc=>standard_crm_api_internal_err.
    ...
    if lv_guid is initial.
      ...
      raise exception type zcx_crm_order_api_exc
        exporting
          textid = zcx_crm_order_api_exc=>cant_get_guid_of_new_instance.
    ...
    endif. " if me->zif_custom_crm_order_api~mv_crm_cat_schema is not initial
    ...
    if ( sy-subrc <> 0 ).
      ...
      raise exception type zcx_crm_order_api_exc
        exporting
          textid = zcx_crm_order_api_exc=>error_saving_document.
    ...
    if <fs_value> is not initial.
      ...
      lo_crm_product = new zcl_crm_product( <fs_value> ).
    ...
    if ( mv_custom_fields_db_table eq 'CRMD_ORDERADM_H' ).
      ...
      if sy-subrc = 0.
        ...
        if  lv_rc <> 0.
          ...
          raise exception type zcx_crm_order_api_exc
            exporting
              textid = zcx_crm_order_api_exc=>error_saving_cust_flds_of_doc.
  ...
  method zif_custom_crm_order_read~get_attachments_list_by_guid.
    ...
    loop at et_attachments_list_short assigning field-symbol(<docs>).
      ...
      if ls_entity-upload_date_formatted is initial.
        ...
        ls_entity-upload_date_formatted = zcl_assistant_utilities=>format_timestamp( ls_entity-upload_date ).
  ...
  method get_texts.
    ...
    if sy-subrc <> 0.
      ...
      raise exception type zcx_crm_order_api_exc
        exporting
          textid  = zcx_crm_order_api_exc=>cant_get_texts_by_guid
          ip_guid = ip_guid.
    ...
    loop at lt_texts assigning field-symbol(<texts>) where stxh-tdid ne 'SUSD'.
      ...
      if lv_html_text_no is initial.
        ...
        loop at lv_stream_text assigning field-symbol(<stream>).
          concatenate ls_text-text <stream> into ls_text-text separated by '<br>'.
        endloop.
  ...
  method get_text_details_odata.
    ...
    concatenate lv_email_subject lv_descripton into lv_email_subject separated by ':'.
  ...
  method zif_custom_crm_order_read~get_attachment_by_keys.
    ...
    ls_loio = value skwf_io( objid = ip_loio class = 'CRM_L_ORD' objtype = 'L' ).
    ls_phio = value skwf_io( objid = ip_phio class = 'CRM_P_ORD' objtype = 'P' ).
    ...
    read table lt_docs assigning field-symbol(<doc>) with key phio = ls_phio loio = ls_loio.
    if sy-subrc = 0.
  ...
  method zif_custom_crm_order_read~get_attachment_content_by_keys.
    ...
    try.
        ls_attachment = me->zif_custom_crm_order_read~get_attachment_by_keys( ip_guid = ip_guid ip_loio = ip_loio ip_phio = ip_phio ).
  ...
  method zif_custom_crm_order_update~delete_attachment.
    ...
    if ls_err_msg is not initial.
      ...
      raise exception type zcx_crm_order_api_exc
        exporting
          textid           = zcx_crm_order_api_exc=>internal_error
  ...
  method zif_custom_crm_order_organizer~is_order_matching_to_search.
