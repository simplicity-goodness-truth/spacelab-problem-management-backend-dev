interface zif_custom_crm_order_init
  public .
  ...
  methods set_db_struct_fields_map
    importing
      it_db_struct_fields_map type zcrm_order_tt_cust_fields_map.

  methods set_custom_fields_db_table
    importing
      ip_custom_fields_db_table type tabname16.

  methods set_process_type
    importing
      ip_process_type type crmt_process_type.

  methods set_structure_name
    importing
      ip_structure_name type strukname.

  methods set_sold_to_party
    importing
      ip_sold_to_party type crmt_partner_no.

  methods set_crm_category1
    importing
      ip_crm_category1 type crm_erms_cat_as_id.

  methods set_crm_category2
    importing
      ip_crm_category2 type crm_erms_cat_as_id.

  methods set_crm_category3
    importing
      ip_crm_category3 type crm_erms_cat_as_id.

  methods set_crm_category4
    importing
      ip_crm_category4 type crm_erms_cat_as_id.

  methods set_crm_cat_schema
    importing
