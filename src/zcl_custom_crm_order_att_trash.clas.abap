class zcl_custom_crm_order_att_trash definition
  public
  final
  create public .
  ...
  public section.
    ...
    interfaces: zif_custom_crm_order_att_trash .
...
class zcl_custom_crm_order_att_trash implementation.
  ...
  method zif_custom_crm_order_att_trash~put_att_to_trash_bin.

    data wa_zcrmo_att_trash type zcrmo_att_trash.

    if ( mv_guid is not initial ) and ( mv_process_type is not initial ) .

      wa_zcrmo_att_trash-guid = zcl_assistant_utilities=>generate_x16_guid(  ).
      wa_zcrmo_att_trash-refguid = mv_guid.
      wa_zcrmo_att_trash-content = ip_content.
      wa_zcrmo_att_trash-processtype = mv_process_type.
      wa_zcrmo_att_trash-filename = ip_file_name.
      wa_zcrmo_att_trash-objectid = me->get_object_id_by_guid(  ).
      wa_zcrmo_att_trash-deletiondate = sy-datum.
      wa_zcrmo_att_trash-deletiontime = sy-uzeit.
      wa_zcrmo_att_trash-deletedby = sy-uname.
      wa_zcrmo_att_trash-mimetype = ip_mime_type.

      insert zcrmo_att_trash from wa_zcrmo_att_trash.

    endif.

  endmethod.

  method zif_custom_crm_order_att_trash~put_att_back_to_crm_order.


    data ls_trash_bin_record type zcrmo_att_trash.

    ls_trash_bin_record = me->zif_custom_crm_order_att_trash~get_trash_bin_record( ip_guid ).
  ...
  method zif_custom_crm_order_att_trash~get_trash_bin_record.
  ...
  method delete_trash_bin_record.
    ...
    delete from zcrmo_att_trash
      where guid = ip_guid.

  endmethod.

  method zif_custom_crm_order_att_trash~empty_trash_bin.
