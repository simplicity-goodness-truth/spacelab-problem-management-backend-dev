...
*             when new zcl_slpm_sla_irt_hist( cs_problem-guid )->zif_slpm_sla_hist~is_there_pending_shift( cs_problem-status )
*                     eq abap_true then me->get_stored_irt_perc(  cs_problem-guid  )
*             else cs_problem-irt_perc
*           ).

      clear: lv_stored_irt_perc, lv_pending_irt_shift_exists.

      new zcl_slpm_sla_irt_hist( cs_problem-guid )->zif_slpm_sla_hist~get_sla_perc_of_pending_shift(
        exporting
...
*       when new zcl_slpm_sla_mpt_hist( cs_problem-guid )->zif_slpm_sla_hist~is_there_pending_shift( cs_problem-status )
*               eq abap_true then me->get_stored_mpt_perc(  cs_problem-guid  )
*       else cs_problem-mpt_perc
*     ).

      clear: lv_stored_mpt_perc, lv_pending_mpt_shift_exists.

      new zcl_slpm_sla_mpt_hist( cs_problem-guid )->zif_slpm_sla_hist~get_sla_perc_of_pending_shift(
        exporting
            ip_status = cs_problem-status
        importing
    ...
    cs_problem-closed = cond abap_bool(
        when me->zif_slpm_data_manager~is_status_a_final_status( cs_problem-status ) then abap_true
            else abap_false
    ).
...
*       lo_slpm_user type ref to zif_slpm_user.
  ...
  method get_prob_guids_through_cache.
    ...
    if mo_slpm_cache_controller is bound.
      ...
      if rt_problems_guids is initial.
        ...
        rt_problems_guids = mo_slpm_problem_api->zif_custom_crm_order_read~get_guids_list(  ).
  ...
  method record_attachment_visibility.
    ...
    data wa_zslpm_att_vsbl type zslpm_att_vsbl.

    wa_zslpm_att_vsbl-guid = ip_guid.
    wa_zslpm_att_vsbl-loio_id = ip_loio.
    wa_zslpm_att_vsbl-phio_id = ip_phio.
    wa_zslpm_att_vsbl-visibility = ip_visibility.

    insert zslpm_att_vsbl from wa_zslpm_att_vsbl.
...
*    ).
  ...
  method zif_slpm_data_manager~fill_cached_prb_calc_flds.
    ...
    if ( mo_active_configuration->get_parameter_value( 'USE_NON_STANDARD_SLA_CALC_IN_SNLRU_CACHE' ) eq 'X').
      ...
      ls_sla_status = mo_slpm_problem_api->zif_custom_crm_order_read~get_sla_status_by_guid( ip_guid ).
...
*    endloop.
    ...
    if lt_crm_guids is not initial.
      ...
      loop at lt_crm_guids assigning field-symbol(<ls_crm_guid>).
        ...
        if ( mo_active_configuration->get_parameter_value( 'USE_SNLRU_CACHE' ) eq 'X').
          ...
          me->zif_slpm_data_manager~fill_cached_prb_calc_flds(
            exporting
  ...
  method zif_slpm_data_manager~get_problem_sla_mpt_history.
    ...
    select guid apptguid problemguid mpttimestamp mpttimezone mptperc update_timestamp update_timezone statusin
        statusout priorityin priorityout username manualchange from zslpm_mpt_hist into corresponding fields of table lt_sla_mpt_history
        where problemguid = ip_guid.
