class zcl_slpm_prob_exp_dec_txt definition
  public
  inheriting from zcl_slpm_prob_exp_dec
  final
  create public .
  ...
  public section.
    ...
    methods:
      ...
      zif_slpm_prob_exp~export_problems redefinition.
  ...
  private section.
    ...
    data:
      ...
      mt_problems_export        type zif_slpm_prob_exp=>ty_problems,
      mv_problems_export_txt    type string,
      mv_problems_export_folder type string value 'C:\tmp\',
      mo_file_system            type ref to zif_file_system.
    ...
    methods:
      ...
      save_problem_exp_txt_to_fs
        importing
          is_problem                    type zif_slpm_prob_exp=>ty_problem
...
class zcl_slpm_prob_exp_dec_txt implementation.

  method zif_slpm_prob_exp~export_problems .

    if it_problems_export is initial.

      mt_problems_export = mo_slpm_prob_exp->zif_slpm_prob_exp~export_problems(  ).
  ...
  method set_file_system.
    ...
    mo_file_system = new zcl_file_system(  ).
  ...
  method save_problem_exp_txt_to_fs.
    ...
    ( field = 'Дата проблемы' value = zcl_assistant_utilities=>format_date( ip_format = 'DD.MM.YYYY' ip_date = is_problem-data-postingdate ) )
    ( field = 'Дата и время создания' value = zcl_assistant_utilities=>format_timestamp( is_problem-data-created_at ) )
    ( field = 'Полное время обработки' value = is_problem-data-totalproctimeminutes )
    ( field = 'Дата и время изменения' value =  zcl_assistant_utilities=>format_timestamp( is_problem-data-changedat ) )
    ...
    ( field = 'Взять в работу до (план)' value = zcl_assistant_utilities=>format_timestamp( is_problem-data-irt_timestamp ) )
    ( field = 'Статус IRT' value = |{ is_problem-data-irt_perc }%| )
    ( field = 'Срок исполнения' value = zcl_assistant_utilities=>format_timestamp( is_problem-data-mpt_timestamp ) )
    ...
    ( field = 'Время взятия проблемы в работу (факт)' value =
        |{ zcl_assistant_utilities=>format_time( is_problem-data-firstreactiontime ) } { zcl_assistant_utilities=>format_date( ip_format = 'DD.MM.YYYY' ip_date = is_problem-data-firstreactiondate ) }| )
    ...
    loop at is_problem-texts assigning field-symbol(<ls_text>).
      ...
      lv_problem_text_string = |{ lv_problem_text_string }| &&
        |{ <ls_text>-tdid_txt } / { zcl_assistant_utilities=>format_timestamp( <ls_text>-date_time_text ) } / { <ls_text>-user_text }: | && lc_new_line &&
