class zcl_custom_crm_order_sla_escal definition
  public
  final
  create public .
  ...
  public section.
    ...
    interfaces zif_custom_crm_order_sla_escal.

  protected section.
  private section.

    data:
      mt_escalation_setup           type table of zcrmo_sla_escal,
      mt_process_types_in_scope     type table of crmt_process_type,
      mt_esc_setup_for_process_type type table of zcrmo_sla_escal,
      mt_variables_values           type zst_text_tt_variables_values,
      ms_crm_order                  type zcrm_order_ts,
      mv_current_sla_value          type int4,
      mo_slpm_product               type ref to zif_slpm_product.
    ...
    methods:
      ...
      process_sla_data
        importing
          ...
          io_crm_order_read type ref to zif_custom_crm_order_read
      ...
      get_email_addresses
        ...
        returning
          value(rt_email_addresses) type zmessenger_tt_addresses,
      ...
      is_order_valid_for_mail_post
        importing
          is_escal_setup  type zcrmo_sla_escal
        returning
...
class zcl_custom_crm_order_sla_escal implementation.


  method add_escal_log_record.

    data wa_zcrmo_sla_esclog type zcrmo_sla_esclog.

    get time stamp field wa_zcrmo_sla_esclog-detectiontimestamp.


    wa_zcrmo_sla_esclog-processtype = ip_process_type.
    wa_zcrmo_sla_esclog-guid = ms_crm_order-guid.
    wa_zcrmo_sla_esclog-slatype = ip_sla_type.
    wa_zcrmo_sla_esclog-object_id = ms_crm_order-objectid.
    wa_zcrmo_sla_esclog-perc = mv_current_sla_value.
    wa_zcrmo_sla_esclog-status = ms_crm_order-status.
    wa_zcrmo_sla_esclog-emailreceivers = ip_emailreceivers.
    wa_zcrmo_sla_esclog-emailsent = ip_email_sent.
    wa_zcrmo_sla_esclog-perchigh = ip_percentage_high.
    wa_zcrmo_sla_esclog-perclow = ip_percentage_low.

    insert zcrmo_sla_esclog from wa_zcrmo_sla_esclog.
  ...
  method get_email_addresses.
    ...
    data:
      ...
      lv_email_address   type zmessenger_address.

    case ip_process_role.

      when 'REQUESTER'.

        lo_bp_address_book = new zcl_bp_master_data( ms_crm_order-requestorbusinesspartner ).
      ...
      when 'PROCESSOR'.
        ...
        lo_bp_address_book = new zcl_bp_master_data( ms_crm_order-processorbusinesspartner ).
        lv_email_address = lo_bp_address_book->get_email_address(  ).
  ...
  method get_order_sent_esc_mail_count.
    ...
    select count( * ) from zcrmo_sla_esclog
        into rp_count
  ...
  method is_mail_post_possible.
    ...
    if is_escal_setup-emailinworkinghours eq abap_true.
      ...
      lo_slpm_product = new zcl_crm_service_product( ms_crm_order-productguid ).

      lv_avail_profile = lo_slpm_product->get_availability_profile_name(  ).

      if lv_avail_profile is not initial.

        lo_serv_profile = new zcl_serv_profile( lv_avail_profile ).
  ...
  method process_escalated_orders.
    ...
    data:
      lo_crm_order_init       type ref to zif_custom_crm_order_init,
      lo_crm_order_read       type ref to zif_custom_crm_order_read,
      lt_guids                type zcrm_order_tt_guids,
    ...
    lo_crm_order_init = new zcl_custom_crm_order_api(  ).
  ...
  method process_sla_data.
    ...
    data:
      ...
      lt_esc_setup_for_sla_type   type standard table of zcrmo_sla_escal,
      lt_statuses_to_skip         type table of j_estat,
      lv_sender_address           type string,
      lv_email_has_been_sent      type abap_bool,
      lt_receiver_email_addresses type zmessenger_tt_addresses.
...
*          lv_percentage_high = mt_esc_setup_for_process_type[ slatype = ip_sla_type ]-perchigh.
  ...
  method zif_custom_crm_order_sla_escal~process_escalations.
  ...
  method get_support_team_addresses.
    ...
    data: lv_team_to_inform         type pd_objid_r,
          ...
          wa_address                type zmessenger_address,
          lo_bp_address_book        type ref to zif_contacts_book.
    ...
    if lv_team_to_inform is not initial.
      ...
      lo_organizational_model = new zcl_organizational_model( lv_team_to_inform ).
      ...
      loop at lt_proc_pool_assigned_pos assigning field-symbol(<ls_proc_pool_assigned_pos>).
        ...
        lo_bp_address_book = new zcl_bp_master_data( <ls_proc_pool_assigned_pos>-businesspartner ).
...
*    rp_org_unit = lo_slpm_product->get_org_unit_for_updates( ).
  ...
  method set_slpm_product.
    ...
    mo_slpm_product = new zcl_slpm_product( ip_product_guid ).
